...
# Test galleries_create_with_memories endpoint functionality
...
test_store_gallery_idempotency() {
    ...
    local result2=$(dfx canister call backend galleries_create_with_memories "($gallery_data, false)" 2>/dev/null)
...
# Test 4: Compare with basic galleries_create
test_compare_with_basic_store_gallery() {
    echo_info "Testing comparison between galleries_create and galleries_create_with_memories..."
    ...
    local result_enhanced=$(dfx canister call backend galleries_create_with_memories "($gallery_data_enhanced, false)" 2>/dev/null)
...
test_gallery_retrieval_after_storage() {
    ...
    local get_result=$(dfx canister call backend galleries_read "(\"$gallery_id\")" 2>/dev/null)
    
    if echo "$get_result" | grep -q "variant {" && echo "$get_result" | grep -q "Ok = record"; then
        ...
        echo_fail "Unexpected response from galleries_read: $get_result"
        return 1
    fi
}
...
test_different_gallery_configurations() {
    echo_info "Testing galleries_create_with_memories with different configurations..."
