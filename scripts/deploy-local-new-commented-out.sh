...
if eval "$DEPLOY_CMD" && dfx deploy internet_identity; then
    ...
    # COMMENTED OUT: Environment variable management is redundant
    # dfx deploy already writes CANISTER_ID_* and DFX_* variables to .env automatically
    # Next.js automatically transforms these to NEXT_PUBLIC_* variables
    # See: docs/issues/now/environment-management-system-analysis.md for detailed analysis
    #
    # # Get canister IDs
    ...
    # Update /etc/hosts - COMMENTED OUT: Not needed due to system-level *.localhost resolution
    # The system automatically resolves *.localhost to 127.0.0.1, making /etc/hosts entries unnecessary
    # See: docs/issues/now/etc-hosts-mapping-redundancy-analysis.md for detailed analysis
    # 
