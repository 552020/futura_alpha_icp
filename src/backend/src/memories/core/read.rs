...
/// Check if a memory exists and is accessible
pub fn memories_exists_core<E: Env, S: Store>(
    env: &E,
    store: &S,
    memory_id: MemoryId,
...
mod tests {
    ...
    #[test]
    fn test_memories_read_core_happy_path() {
        ...
        // Test read
        let result = memories_read_core(&env, &store, memory_id.clone());

        match result {
            Ok(memory) => {
                assert_eq!(memory.id, memory_id);
                assert_eq!(memory.metadata.title, Some("Test Memory".to_string()));
            }
            Err(e) => panic!("Memory read should succeed: {:?}", e),
        }
    }
    ...
    fn test_memories_read_core_not_found() {
        ...
        // Test read non-existent memory
        let result = memories_read_core(&env, &store, memory_id);

        match result {
            Ok(_) => panic!("Memory read should fail for non-existent memory"),
    ...
    #[test]
    fn test_memories_read_core_unauthorized() {
        let mut store = MockStore::new();
        let owner = crate::types::PersonRef::Principal(
            Principal::from_text("rdmx6-jaaaa-aaaah-qcaiq-cai").unwrap(),
        );
        ...
        // Test read with unauthorized user
        let result = memories_read_core(&env, &store, memory_id);

        match result {
            Ok(_) => panic!("Memory read should fail for unauthorized user"),
    ...
    #[test]
    fn test_memories_exists_core_happy_path() {
        let mut store = MockStore::new();
        let owner = crate::types::PersonRef::Principal(
            Principal::from_text("rdmx6-jaaaa-aaaah-qcaiq-cai").unwrap(),
        );
        ...
        // Test exists
        let result = memories_exists_core(&env, &store, memory_id.clone());

        match result {
            Ok(exists) => assert!(exists),
            Err(e) => panic!("Memory exists check should succeed: {:?}", e),
        }
    }

    #[test]
    fn test_memories_exists_core_not_found() {
        let mut store = MockStore::new();
        let owner = crate::types::PersonRef::Principal(
            Principal::from_text("rdmx6-jaaaa-aaaah-qcaiq-cai").unwrap(),
        );
        ...
        // Test exists for non-existent memory
        let result = memories_exists_core(&env, &store, memory_id);
