...
/**
 * Debug test for Asset Retrieval failure in 2-lane + 4-asset system
 * This test isolates the failing "Asset Retrieval" test to understand why Lane B is being rejected
 */
...
if (!BACKEND_CANISTER_ID) {
  ...
  console.error("Usage: node test_asset_retrieval_debug.mjs <BACKEND_CANISTER_ID>");
  process.exit(1);
}
...
// Helper functions are now imported from helpers.mjs

// Upload function with detailed error handling
async function uploadFileToICP(actor, fileBuffer, fileName, onProgress) {
...
async function processDerivatives(actor, fileBuffer, fileName, onProgress) {
  try {
    echoInfo(`🖼️ Processing derivatives for ${(fileBuffer.length / (1024 * 1024)).toFixed(1)} MB file`);
...
// Main test function
async function testAssetRetrievalDebug() {
  try {
    echoInfo("🔍 Starting Asset Retrieval Debug Test");
    echoInfo("This test isolates the failing Asset Retrieval test to understand why Lane B is being rejected");
    ...
    // Test 1: Individual Lane A (Original Upload)
    echoInfo("\n🧪 Test 1: Individual Lane A (Original Upload)");
    try {
      const laneAResult = await uploadFileToICP(actor, fileBuffer, fileName);
      echoSuccess(`✅ Lane A completed: ${laneAResult.memoryId}`);
    } catch (error) {
      echoError(`❌ Lane A failed: ${error.message}`);
      throw error;
    }

    // Test 2: Individual Lane B (Derivatives Processing)
    echoInfo("\n🧪 Test 2: Individual Lane B (Derivatives Processing)");
    try {
      ...
      echoError(`❌ Lane B failed: ${error.message}`);
      throw error;
    }

    // Test 3: Parallel Execution (This is where the failure occurs)
    echoInfo("\n🧪 Test 3: Parallel Execution (Asset Retrieval Simulation)");
    try {
    ...
    echoSuccess("\n🎉 Asset Retrieval Debug Test completed successfully!");
  } catch (error) {
    echoError(`\n💥 Asset Retrieval Debug Test failed: ${error.message}`);
    process.exit(1);
  }
}

// Run the test
testAssetRetrievalDebug();
