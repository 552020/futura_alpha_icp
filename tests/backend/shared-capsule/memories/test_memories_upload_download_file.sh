...
# Usage: ./test_memories_upload_download_file.sh <file_path>
...
OUTPUT_DIR="tests/backend/shared-capsule/memories/assets/output"
...
test_file_upload_download() {
    ...
    if [[ "$UPLOAD_METHOD" == "inline" ]]; then
        ...
        local memory_data=$(create_inline_memory_data "$file_path")
        local idem="test_inline_$(date +%s)"
        
        echo_debug "Inline upload command: dfx canister call $CANISTER_ID memories_create \"(\\\"$capsule_id\\\", $memory_data, \\\"$idem\\\")\""
        local result=$(dfx canister call $CANISTER_ID memories_create "(\"$capsule_id\", $memory_data, \"$idem\")" 2>/dev/null)
